...
class Solution:
    def batch_loader(self, raw_dataset: str, context_length: int, batch_size: int) -> Tuple[List[List[str]], List[List[str]]]:
        ...
        starts = torch.randint(len(words)-context_length,(batch_size,))
        X = [words[start:start+context_length] for start in starts]
        Y = [words[start+1:start+1+context_length] for start in starts]
        return X,Y
